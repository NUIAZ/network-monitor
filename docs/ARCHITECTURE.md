...
3. **Execute.** `NmapExecutorService` validates the CIDR, fixes up the argument string (probe injection, `--open` stripping — see README "How the scanning works"), writes XML to a GUID-named temp file so parallel runs can't collide, and drains stdout/stderr concurrently (sequential reads deadlock on chatty scans).
